...
/*   Updated: 2026/05/25 21:41:21 by agnesgar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_vertically(t_game *game, double speed, t_data *data)
{
	if (mlx_is_key_down(game->mlx, MLX_KEY_W))
	{
		if (data->map[(int)(data->player.y)]
			[(int)(data->player.x + data->player.dir_x * (speed + MARGIN))] != '1')
			data->player.x += data->player.dir_x * speed;
		if (data->map[(int)(data->player.y + data->player.dir_y * (speed + MARGIN))]
			[(int)(data->player.x)] != '1')
...
void	move_left_or_right(t_game *game, double olddir_x, double oldplane_x, double rot, t_data *data)
{
	if (mlx_is_key_down(game->mlx, MLX_KEY_LEFT))
	{
		data->player.dir_x = olddir_x * cos(-rot) - data->player.dir_y * sin(-rot);
		data->player.dir_y = olddir_x * sin(-rot) + data->player.dir_y * cos(-rot);
		data->player.plane_x = oldplane_x * cos(-rot)
...
void	move_horizontally(t_game *game, double speed, t_data *data)
{
	...
	{
		if (data->map[(int)(data->player.y)]
			[(int)(data->player.x + data->player.dir_y * (speed + MARGIN))] != '1')
			data->player.x += data->player.dir_y * speed;
		if (data->map[(int)(data->player.y - data->player.dir_x * (speed + MARGIN))]
			[(int)(data->player.x)] != '1')
